...
cartesian_visibility_profile <- function(extinction_profile,model=NULL,wavelength,incoming=FALSE,incoming_distance=NULL,incoming_height=NULL,output_files=TRUE,verbose=TRUE)
{
  ...
  if (!is.null(incoming_height) && !is.null(incoming_distance))
  {
    ...
    if (vertical_visibility[1] < incoming_height)    
    {
      ...
      pseudo_visibility <- visibility_range(extinction=c(rep(max(cartesian_profile[1:ceiling(incoming_height/bin_width),ceiling(incoming_distance/bin_width)]),maximum_height/bin_width),cartesian_profile[1:ceiling(incoming_height/bin_width),ceiling(incoming_distance/bin_width)]),bin_width,model,wavelength,incoming=TRUE,incoming_height+maximum_height,verbose=FALSE)[1]
  ...
  if (output_files)
  {
    ...
    if (!is.null(incoming_height) && !is.null(incoming_distance))
    {
      if (incoming)
      {
        write.table(c(horizontal_visibility,vertical_visibility[1],slant_visibility,homogeneous_slant_visibility),file=paste("Incoming_object_visibility_",model,".txt",sep=""),quote=FALSE,sep="\t",col.names=paste("Incoming height: ",incoming_height," m, incoming distance: ",incoming_distance," m.",sep=""),row.names=c("Horizontal visibility: ","Vertical visibility: ","Slant visibility:","Homogeneous slant visibility:"))
      } else {
